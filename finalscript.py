...
class GraphicalPasswordApp(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for F in (StartPage, RegistrationPage, LoginPage, PasswordPageOne, PasswordPageTwo,
                  ...
                  LoginPageTwo, LoginPageThree, LoginPageFour, LoginPageFive, OTPPage):
...
class OTPPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label = tk.Label(frame, bg="white", text="PLEASE VERIFY YOUR EMAIL ADDRESS HERE", font=("Arial", 30), pady=30)
        label.pack(pady=10,padx=10)

        label2 = tk.Label(frame, text = "ENTER EMAIL", bg="white")
        label2.pack()

        email = tk.Entry(frame, bd=2)
        email.pack(pady=5,ipadx=50, ipady=5)
        ...
        def sendotp():
            ...
            def send_otp(email, otp):
                sender_email = "umanjunath7210@gmail.com"
                sender_password = "oyal guap bqyz erqo"
                smtp_server = "smtp.gmail.com"
                smtp_port = 587
                msg = MIMEMultipart()
                msg['From'] = sender_email
                msg['To'] = email
                msg['Subject'] = "Graphical Password App"
            ...
            messagebox.showinfo("Alert", "Please wait! We are sending OTP on email address!")
                       
            generated_otp = generate_otp()
                
            res = send_otp(email_data, generated_otp)

            pickle_out = open("cred.pickle", "wb")
            pickledata = {'generated_otp': generated_otp}
            pickle.dump(pickledata, pickle_out)
            pickle_out.close()

            pickle_out = open("fp.pickle", "wb")
            pickledata = {'email_data': email_data}
            pickle.dump(pickledata, pickle_out)
            pickle_out.close()
                
            messagebox.showinfo("Alert", "OTP sent successfully.")

        button2 = tk.Button(frame, text="SEND OTP", width=10, height=1, bg = "firebrick", foreground = "white",command=sendotp)
        button2.pack(pady=20, anchor='w', padx=650)

        label3 = tk.Label(frame, text = "ENTER OTP", bg="white")
        label3.pack()

        otp = tk.Entry(frame, bd=2)
        otp.pack(pady=5,ipadx=50, ipady=5)

        def callback():
            pickle_in = open("cred.pickle", "rb")
            pickledata = pickle.load(pickle_in)
            generated_otp = pickledata.get('generated_otp')
            ...
            if len(otp_data) == 0:
                messagebox.showinfo("Alert", "Please enter OTP.")
            elif otp_data != generated_otp:
                messagebox.showinfo("Alert", "Please enter correct OTP.")
            else:
                controller.show_frame(PasswordPageOne)

        button3 = tk.Button(frame, text="SUBMIT", width=10, height=1, bg = "firebrick", foreground = "white",
                            command=callback)
        button3.pack(pady=20, anchor='w', padx=650)
...
##                print("count"+str(count))
                if count <=1:
                    count +=1
                    messagebox.showinfo("Alert!", "Please select correct point! You have only two attempt!")
